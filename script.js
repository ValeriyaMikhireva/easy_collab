...
function applyFilters() {
    ...
    const selectedBarterFilter = getSelectedBarterFilter();
    
    filteredCompanies = companies.filter(company => {
        const matchesSearch = matchesSearchTerm(company, searchTerm);
        const matchesDirection = selectedDirection === 'all' || 
                                company['Направление деятельности']?.toLowerCase().includes(selectedDirection.toLowerCase());
        const matchesBarter = selectedBarterFilter === 'all' || 
                             checkBarterMatch(company, selectedBarterFilter);
        
        return matchesSearch && matchesDirection && matchesBarter;
    });
...
function displayCompanies() {
    ...
    if (!container) {
        console.error('Контейнер companiesContainer не найден');
        return;
    }
...
function updateResultsCount() {
    ...
    if (countElement) {
        ...
        const declension = getDeclension(count, ['компания', 'компании', 'компаний']);
        countElement.textContent = `Найдено ${count} ${declension}`;
    }
}
...
function openCompanyModal(companyName) {
    ...
    const modalContent = document.getElementById('modalCompanyInfo');
    
    if (!modal || !modalContent) return;
    
    modalContent.innerHTML = createModalContent(company);
